...
# Swift Packages Feed Generator Docs

Swift Packages Feed Generator provides a set of Swift packages and tooling for the generation
and consumption of package feeds.

To get started with Swift Packages Feed Generator, [`import PackageFeedModel`](../PackageFeedModel/index.html).
EOF
...
jazzy_args=(--clean
            --author 'Swift Packages Feed Generator team'
            --readme "$module_switcher"
            --author_url https://github.com/apple/swift-packages-feed-generator
            --github_url https://github.com/apple/swift-packages-feed-generator
            --github-file-prefix https://github.com/apple/swift-packages-feed-generator/tree/$version
            --theme fullwidth
...
for module in "${modules[@]}"; do
  args=("${jazzy_args[@]}" --output "$jazzy_dir/docs/$version/$module" --docset-path "$jazzy_dir/docset/$version/$module"
        ...
        --root-url "https://apple.github.io/swift-packages-feed-generator/docs/$version/$module/")
  if [[ "$(uname -s)" == "Linux" ]]; then
...
if [[ $PUSH == true ]]; then
  ...
  git add --all docs
  echo '<html><head><meta http-equiv="refresh" content="0; url=docs/current/PackageFeedModel/index.html" /></head></html>' > index.html
  git add index.html
  touch .nojekyll
  git add .nojekyll
